...
| Docker Compose | [docker-compose.dev.yml](../../docker-compose.dev.yml), [infra/docker-compose.dev.yml](../../infra/docker-compose.dev.yml) | Traefik + dev-server (ADR#8) |
...
- `docker-compose.dev.yml`, `infra/docker-compose.dev.yml` — Traefik + dev-server
- `docs/dev/DEV_DOMAIN.md` — hosts, ports, commands, /health

**Commands:**
- `pnpm dev` — dev server on :5173
- `curl http://localhost:5173/health` → `{"status":"ok"}`
- `docker compose -f docker-compose.dev.yml up -d` — Traefik + dev-server (from repo root)
...
- `pnpm test` — unit (Vitest, front/__tests__/fp1/)
...
- [x] unit PASS (14)
- [x] e2e PASS (10)
- [x] События эмитятся в ключевых точках (window_*, drag_end, resize_end, taskbar_click, app_ready, handshake_timeout, message_rejected)
