...
| T-FP4-M2-\*        | `signed-url-content-type-m2.integration.test.ts` | All media types: 200 + expected Content-Type                                 |
...
| T-FP4-M0-PAYLOAD-SCHEMA | `viewer-open-file-handshake.test.ts` | OpenFilePayload schema includes expiresAt for signed URL audit                 |
...
| `viewer-open-file-sets-src.test.ts`  | OPEN_FILE sets img#viewer-img src                                                 |
| `app-host-handshake.test.tsx`        | APP_READY from origin "null" → handshake; unknown source rejected                 |
| `viewer-bootstrap-sandbox.test.ts`   | T-FP4-M0-BOOT: viewer APP_READY uses targetOrigin \* (not window.location.origin) |
| `viewer-open-file-handshake.test.ts` | T-FP4-M0-PAYLOAD-SCHEMA: OpenFilePayload includes expiresAt for audit             |
...
| `signed-url-fetchable.integration.test.ts`       | T-FP4-M0-CURL: sample.webp; T-FP4-M0-MP3: sample.mp3 (Media Player) |
| `signed-url-content-type-m2.integration.test.ts` | T-FP4-M2: all media Content-Type                                    |
| `playlist-flow.integration.test.ts`              | list + open-url → playlist N>=1                                     |
