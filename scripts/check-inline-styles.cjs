...
const ALLOW_TAG_V2_PATTERN =
  /inline-style:\s*allowed\s*\(\s*reason:\s*(drag\/resize|layout-calc|performance)\s*;\s*why:\s*[^;)]+\s*;\s*revisit:\s*[^)]+\s*\)/i;
const ALLOW_TAG_V2_REASON_CAPTURE =
  /inline-style:\s*allowed\s*\(\s*reason:\s*(drag\/resize|layout-calc|performance)\s*;/i;

// Legacy v1 (missing why/revisit) - used to detect and reject
const ALLOW_TAG_V1_PATTERN =
  /inline-style:\s*allowed\s*\(reason:\s*(drag\/resize|layout-calc|performance)\s*\)/i;
...
const DISALLOWED_BACKDROP_PATTERN =
  /position:\s*["']fixed["'].*?(?:inset|top|left|right|bottom):\s*0|(?:inset|top|left|right|bottom):\s*0.*?position:\s*["']fixed["']/is;
    ...
    if (line.includes("style={{") || line.includes('style={{"')) {
      ...
      if (absoluteUnitsMatch) {
        const units = [
          ...new Set(
            absoluteUnitsMatch
              .map((m) => m.match(/(px|pt|pc|in|cm|mm|[qQ])$/i)?.[0])
              .filter(Boolean)
          ),
        ];
        errors.push({
...
if (allErrors.length > 0) {
  ...
  console.error("  Rules:");
  console.error(
    "    - Inline styles require allow-tag v2: (reason: X; why: <runtime dynamic>; revisit: <milestone>)"
  );
  console.error("    - Disallowed: fixed fullscreen backdrop, zIndex magic numbers (4+ digits)");
  console.error(
    "    - Absolute units (px, pt, pc...) forbidden except px in transform/translate for drag.\n"
  );
  process.exit(1);
} else {
