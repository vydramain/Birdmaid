...
async function onItemDblClick(item: FsItem): Promise<void> {
  if (item.kind === "file") {
    ...
    log.warn("[Explorer] Unsupported file type, no handler:", item.path);
    return;
  }
...
async function onNewFolder(): Promise<void> {
  ...
  if (!result.ok) {
    div.remove();
    currentListItems.pop();
    log.error("[Explorer] create-folder failed");
    return;
  }
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (res.status === 201) {
        ...
        log.error("[Explorer] upload failed:", res.status, await res.text());
      }
    } catch (e) {
      div.remove();
      currentListItems.splice(currentListItems.indexOf(placeholder), 1);
      log.error("[Explorer] upload error:", e);
    }
    reindexListTiles();
  }
}
...
async function onUploadZipChange(): Promise<void> {
  ...
  try {
    ...
    if (res.status === 201) {
      ...
      div.remove();
      currentListItems.splice(currentListItems.indexOf(placeholder), 1);
      log.error("[Explorer] upload-zip-app failed:", res.status, await res.text());
    }
  } catch (e) {
    div.remove();
    currentListItems.splice(currentListItems.indexOf(placeholder), 1);
    log.error("[Explorer] upload-zip-app error:", e);
  }
  reindexListTiles();
}
...
async function onDelete(item: FsItem): Promise<void> {
  ...
  try {
    ...
    if (res.status === 204) {
      ...
      spinner.replaceWith(restored);
      log.error("[Explorer] delete failed:", res.status, await res.text());
    }
  } catch (e) {
    tile.removeAttribute("aria-disabled");
    tile.style.pointerEvents = "";
    ...
    restored.textContent = oldName;
    spinner.replaceWith(restored);
    log.error("[Explorer] delete error:", e);
  }
}
...
async function commitRename(
  ...
  try {
    ...
    if (res.ok) {
      ...
      labelEl.textContent = oldName;
      tile.appendChild(labelEl);
      log.error("[Explorer] rename failed:", res.status, await res.text());
    }
  } catch (e) {
    spinner.remove();
    tile.removeAttribute("aria-disabled");
    ...
    labelEl.textContent = oldName;
    tile.appendChild(labelEl);
    log.error("[Explorer] rename error:", e);
  }
}
