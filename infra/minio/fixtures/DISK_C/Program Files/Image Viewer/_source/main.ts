...
function showImage(url: string): void {
  ...
  log.debug("[ImageViewer] fetch started:", url);
  ...
  const timeoutId = setTimeout(() => {
    if (resolved) return;
    log.debug("[ImageViewer] load timeout");
    log.error("[ImageViewer] Load timeout (CORS/network?):", url);
    resolve();
  ...
  fetch(url, { mode: "cors" })
    ...
    .catch((err) => {
      if (resolved) return;
      clearTimeout(timeoutId);
      log.debug("[ImageViewer] fetch failed:", err);
      log.error("[ImageViewer] Failed to load image:", url, err);
      resolve();
      error.style.display = "block";
      error.textContent = "Unable to load image";
      img.style.display = "none";
...
function handleOpenFile(payload: {
  ...
  log.debug("[ImageViewer] OPEN_FILE received");
